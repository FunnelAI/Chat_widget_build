fetch(
    "https://raw.githubusercontent.com/FunnelAI/Chat_widget_build/test/redesign/build/asset-manifest.json"
  ).then((res) => {
    res.text().then((d) => {
      const files = JSON.parse(d);
      const mainChunk = document.createElement("script");
      mainChunk.src = `https://cdn.jsdelivr.net/gh/FunnelAI/Chat_widget_build/test/redesign/build${files.files["main.js"]}`;
      document.head.appendChild(mainChunk);
      const mainChunkCss = document.createElement("link");
      mainChunkCss.href = `https://cdn.jsdelivr.net/gh/FunnelAI/Chat_widget_build/test/redesign/build${files.files["main.css"]}`;
