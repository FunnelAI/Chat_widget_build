fetch(
  "https://raw.githubusercontent.com/FunnelAI/Chat_widget_build/staging/build/asset-manifest.json",
  {
    cache: "no-cache",
  }
).then((res) => {
  res.text().then((d) => {
    ...
    mainChunk.src = `https://cdn.jsdelivr.net/gh/FunnelAI/Chat_widget_build@staging/build${files.files["main.js"]}`;
    document.head.appendChild(mainChunk);
    const mainChunkCss = document.createElement("link");
    mainChunkCss.href = `https://cdn.jsdelivr.net/gh/FunnelAI/Chat_widget_build@staging/build${files.files["main.css"]}`;
